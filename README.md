...
- Опечатка `/auth/sing-up`, `/auth/sing-in` (должно быть `sign-up`/`sign-in`) — сохранена для обратной совместимости с клиентами без отдельного релиза.
- `UserController.getAllUsers()` не имеет `@GetMapping` — маршрут фактически недоступен, требует доработки.
- `POST /api/users/batch` не имеет `@PreAuthorize` с конкретными ролями — доступен любому аутентифицированному принципалу; это осознанное решение для межсервисных вызовов, но стоит учитывать при аудите.
